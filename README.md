<h1 align="center">Madara Uchiwa Anna.D Bot V2</h1>


<p align="center">
    <img alt="Madara" src="./madara-logo.png">
</p>
<p align="center">
    Un bot WhatsApp multi-appareil développé par Madara. N'oubliez pas de laisser une ⭐ (star) pour le projet.
</p>
...
<p align="center">
    ...
    <a href="https://github.com/MadaraUchiwaDevs/Madara-Uchiwa-AnnaD-Bot-V2
)stargazers">
        <p align="center">
 <a href="https://github.com/MadaraUchiwaDevs/Madara-Uchiwa-AnnaD-Bot-V2/stargazers">
  <img src="https://img.shields.io/github/stars/MadaraUchiwaDevs/Madara-Uchiwa-AnnaD-Bot-V2?style=flat-square" alt="Stars" />
</p>
      <a href="https://github.com/MadaraUchiwaDevs/Madara-Uchiwa-AnnaD-Bot-V2
)stargazers">
  <img src="https://img.shields.io/github/forks/MadaraUchiwaDevs/Madara-Uchiwa-AnnaD-Bot-V2?style=flat-square" alt="Forks" />
</a>
</p>
</p>
---
<details>
  <summary>🚀🚀 Déploiement de Madara Uchiwa Anna.D Bot
 V2</summary>

### 🧬 Étape 1 : Fork du dépôt GitHub  
[![Fork GitHub](https://img.shields.io/badge/Fork%20le%20Repo-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/MadaraUchiwaDevs/Madara-Uchiwa-AnnaD-Bot-V2/fork)
---

### 🔐 Étape 2 : Générer une SESSION ID

📌 **Conserve la Session-ID dans un endroit sécurisé.** 
[![Obtenir SESSION-ID](https://img.shields.io/badge/Obtenir%20SESSION--ID-0A0A0A?style=for-the-badge&logo=key&logoColor=white)](https://madara-session-web.onrender.com/)/)
---
...
function setupProject() {
  if (!existsSync('madara')) {
    const clone = spawnSync(
      'git',
      ['clone', 'https://github.com/MadaraUchiwaDevs/Madara-Uchiwa-AnnaD-Bot-V2.git', 'madara'],
      { stdio: 'inherit' }
    );
    if (clone.status !== 0) process.exit(1);
  }

  if (!existsSync('madara/.env')) {
    mkdirSync('madara', { recursive: true });
    writeFileSync('madara/.env', env_file);
    console.log("✅ Fichier .env créé avec succès.");
  }

  const install = spawnSync('npm', ['install'], { cwd: 'madara', stdio: 'inherit' });
  if (install.status !== 0) process.exit(1);
}

function validateSetup() {
  if (!existsSync('madara/package.json')) {
    process.exit(1);
  }

  const check = spawnSync('npm', ['ls'], { cwd: 'madara', stdio: 'ignore' });

  if (check.status !== 0) {
    const reinstall = spawnSync('npm', ['install'], { cwd: 'madara', stdio: 'inherit' });
...
function launchApp() {
  const pm2 = spawn(
    'npx',
    ['pm2', 'start', 'Ovl.js', '--name', 'madara-bot', '--attach'],
    {
      cwd: 'madara',
  ...
  pm2.stderr?.on('data', (chunk) => {
    ...
    if (output.includes('restart')) {
      ...
      if (restartAttempts > 3) {
        spawnSync('npx', ['pm2', 'delete', 'madara-bot'], {
          cwd: 'madara',
          stdio: 'inherit'
        });
        startNodeFallback();
  ...
  pm2.on('exit', startNodeFallback);
  pm2.on('error', startNodeFallback);
}

function startNodeFallback() {
  const child = spawn('node', ['Ovl.js'], {
    cwd: 'madara',
    stdio: 'inherit'
  });
  ...
  child.on('exit', () => {
    ...
    if (crashCount > crashLimit) return;
    startNodeFallback();
  });
}
...
```yaml
name: Madara-Uchiwa Anna.D-Bot-V2 CI
...
MODE=public
NOM_BOT=🤖 Madara Uchiwa Anna.D Bot V2
BOT_NAME=MadaraUchiwaDevs
SESSION_ID=
STICKER_PACK_NAME=Madara-MD
STICKER_AUTHOR_NAME=MADARA✨
```
...
---
### 🙌 Remerciements
- Benjamin – pour son soutien et aide dans la gestion du bot au support.
-St Bible Black - pour son soutien 
- Jaden – pour son soutien.
---
### 📄 Licence
